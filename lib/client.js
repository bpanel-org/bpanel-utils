...
class BpanelClient extends Client {
  /**
  ...
  setWalletClient(_path = null) {
    const path = this.getPath('wallet');
  ...
  setMultisigClient(_path = null) {
    const path = this.getPath('multisig');
...
class ClientOptions {
  ...
  fromOptions(options) {
    ...
    if (path) {
      assert(typeof path === 'string');
      this.path = path;
    } else if (!path && this.id) {
      // if there is no custom path but we have an id
      // then we will point our client at our proxy
      // for the client with that id
